...
- **Optimización VAD y Cierre Rápido de Micrófono**: Se inyectaron los temporizadores `EXTRA_SPEECH_INPUT_COMPLETE_SILENCE_LENGTH_MILLIS = 700ms` y `500ms` en `AndroidSpeechRecognizer.kt`, junto con la reducción del timeout inactivo a 3s (`NO_SPEECH_TIMEOUT_MS`) y retención de silencio a 380ms (`SILENCE_HOLD_MS`) en `AiConversation.kt`. Esto corta la grabación inmediatamente al finalizar la orden de voz (~0.6s de silencio) acelerando las respuestas de la IA.
