...
- **Control Estricto de Intentos Automáticos de Conexión y Ahorro de Batería (Disconnect/Connect)**: Al presionar "Disconnect" (`ACTION_STOP`), el cliente inhabilita por completo los intentos automáticos de conexión/reconexión en memoria y en preferencias persistentes (`Prefs.autoReconnectEnabled`), interrumpe los bucles de reconexión RFCOMM (`scheduleReconnect`), detiene el escáner BLE y finaliza el Foreground Service `MyvuService` sin permitir reinicios en `onTaskRemoved`. Además, cierra inmediatamente los proxies de perfiles de audio Bluetooth (`AudioProfiles.close()`) para evitar sondeos innecesarios de la pila Bluetooth en el teléfono y las gafas, maximizando la duración de la batería mientras no hay conexión. Al presionar "Connect" (`ACTION_START`), el control reactiva `autoReconnectEnabled` y reanuda los escaneos y la secuencia de reconexión.
...
- **Optimización de Timeouts y Consolidación de Peticiones IA (`NoteAiProcessor.kt` / `AiHttpClient.kt` / `OpenAiTranscriptionClient.kt`)**: Peticiones unificadas en 1 sola llamada JSON multi-campo (resumen + tareas + mapa mental + tags) reduciendo latencia en un 80%, con timeouts extendidos a 240s para endpoints locales (`127.0.0.1` / Termux) y reuso de caché de prompts en `llama-server` (`--cache-reuse 256`).
...
- **Compatibilidad Transparente con Gemma 4 y Servidores Locales (`LocalAiClient.kt`)**: Integración del prompt de sistema dentro del primer turno de usuario (`role: "user"`) resolviendo la incompatibilidad de las plantillas GGUF de Gemma 4/MiniCPM en `llama-server` y garantizando que la IA responda y ejecute todas las reglas agénticas correctamente en ejecución local.
- **Soporte Nativo y Compilación con JDK 21 (`app/build.gradle.kts` / `gradle.properties`)**: Configuración completa del compilador Java 21 (`JavaVersion.VERSION_21`), `jvmTarget = "21"` y Kotlin 2.1.10 para aprovechar las optimizaciones del runtime `jdk-21.0.6+7`.
