...
- **Protocolo de Reposo Profundo de Flyme XR**:
  ...
  - **Supresión Definitiva del Bucle Ping-Pong RFCOMM**:
    - **Cooldown de Reapertura (`SPP_COOLDOWN_MS = 15000L`)**: Tras el cierre del servidor SPP por las gafas, `ConnectionManager` registra `lastSppServerClosedAt`. Si el daemon BLE de las gafas emite ráfagas repetidas de `CMD_SPP_SERVER_REQUEST_CONNECT` (71) o `CMD_SPP_SERVER_REQUEST_STATE_OPEN` (72) dentro de la ventana de 15 segundos sin que haya una funcionalidad activa que requiera el relay, las solicitudes son descartadas (`handleSppOpenRequest`), previniendo ciclos de conexión/cierre cada 2 segundos.
    - **RFCOMM Estrictamente Bajo Demanda (`isRelayFeatureActive`)**: El enlace RFCOMM de Bluetooth clásico (BR/EDR) se mantiene suspendido cuando sólo se transmiten notificaciones, telemetría o ajustes (los cuales viajan con consumo mínimo por BLE). Se activa bajo demanda exclusivamente cuando se inicia el uso del trackpad (`trackpadStart`), se graba audio por el micrófono para el asistente de IA o el usuario ejecuta una acción explícita (`wakeRelay(force = true)`).
    - **Modo Pasivo Protegido en `RelaySupervisor.wake(force: Boolean = false)`**: Si `sppServerSuspended` está activo porque las gafas solicitaron entrar en modo pasivo de bajo consumo, cualquier invocación no forzada (`force = false`) es ignorada para no violar el estado de ahorro de energía. Únicamente eventos explícitos con `force = true` reactivan el supervisor.
