...
- **Local-First AI assistant** — press the glasses' button or speak; speech-to-text, an LLM answer, and text-to-speech back to the glasses:
  ...
  - **Extensible Skill Engine (`SKILL.md`)**: Dynamic YAML frontmatter skill loader and parser supporting `SKILL.md` manifests (`skills/built-in/...`) for phone calls, email sending, WhatsApp, Telegram, and `gemini-live-assistant` for full-duplex voice interaction with Google Gemini Multimodal Live API via glasses mic & clean plain text AR HUD. See [docs/SKILLS.md](docs/SKILLS.md) for full details.
